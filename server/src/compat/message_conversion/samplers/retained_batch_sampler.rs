...
impl BinarySchemaSampler for RetainedMessageBatchSampler {
    async fn try_sample(&self) -> Result<BinarySchema, CompatError> {
        let mut index_file = file::open(&self.index_path).await.with_error(|_| {
            format!(
                "{COMPONENT} - failed to open index path: {}",
                self.index_path
            )
        })?;
        let mut log_file = file::open(&self.log_path).await.with_error(|_| {
            format!(
                "{COMPONENT} - failed to open log path: {}",
                self.log_path
            )
        })?;
        let log_file_size = log_file
            ...
            .with_error(|_| {
                format!(
                    "{COMPONENT} - failed to get log file metadata: {}",
                    self.log_path
                )
            })?
            .len();
        ...
        let _ = index_file.read_u32_le().await.with_error(|_| {
            format!(
                "{COMPONENT} - failed to read first index entry from index path: {}",
                self.index_path
            )
        })?;
        let _ = index_file.read_u32_le().await.with_error(|_| {
            format!("{COMPONENT} - failed to read second index entry from index path: {}", self.index_path)
        })?;
        let second_index_offset = index_file.read_u32_le().await.with_error(|_| {
            format!(
                "{COMPONENT} - failed to read third index entry from index path: {}",
                self.index_path
            )
        });
        let second_end_position = index_file.read_u32_le().await.with_error(|_| {
            format!("{COMPONENT} - failed to read fourth index entry from index path: {}", self.index_path)
        });
        ...
        if second_index_offset.is_err() && second_end_position.is_err() {
            let _ = log_file.read_to_end(&mut buffer).await.with_error(|_| {
                format!(
                    "{COMPONENT} - failed to read log file to end: {}",
                    self.log_path
                )
            })?;
        } else {
            ...
            let _ = log_file.read_exact(&mut buffer).await.with_error(|_| {
                format!(
                    "{COMPONENT} - failed to read exact range from log file: {}",
                    self.log_path
                )
            })?;
        }
        let batch = RetainedMessageBatchSnapshot::try_from(Bytes::from(buffer)).with_error(|_| {
                "{COMPONENT} - failed to create RetainedMessageBatchSnapshot from buffer".to_string()
            })?;
