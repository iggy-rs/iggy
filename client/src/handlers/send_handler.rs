...
pub async fn handle(input: &[&str], socket: &UdpSocket, buffer: &mut [u8; 1024]) -> io::Result<()> {
    if input.len() != PARTS {
        return Err(io::Error::new(io::ErrorKind::Other, format!("Invalid send command, expected {} parts.", PARTS)));
    }
    ...
    socket.send([COMMAND, topic, key_kind, key_value, payload].concat().as_slice()).await?;
