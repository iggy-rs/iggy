...
pub async fn handle(input: &str, socket: &UdpSocket, buffer: &mut [u8; 1024]) -> io::Result<()> {
    ...
    match command {
        ...
        _ => {
            if parts.len() < 2 {
                return Err(io::Error::new(io::ErrorKind::Other, "Invalid command parts."));
            }

            let input = &parts[1..];
            match command {
                "create_topic" => handlers::create_topic_handler::handle(input, socket, buffer).await,
                "delete_topic" => handlers::delete_topic_handler::handle(input, socket, buffer).await,
