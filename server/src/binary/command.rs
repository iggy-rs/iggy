...
async fn try_handle(
    ...
    match command {
        ...
        Command::GetGroup(command) => get_group_handler::handle(command, sender, system).await,
        Command::GetGroups(command) => get_groups_handler::handle(command, sender, system).await,
