...
pub async fn handle(
...

    {
        ...
        system
            ...
            .await
            .with_error(|_| {format!("Failed to delete personal access token with name: {token_name}, session: {session}")})?;
    }
    ...
    system
        ...
        .await
        .with_error(|_| {format!("Failed to apply delete personal access token with name: {token_name}, session: {session}")})?;
