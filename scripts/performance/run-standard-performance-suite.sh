...
LARGE_BATCH_FORCED_CACHE_SEND=$(construct_bench_command "$IGGY_BENCH_CMD" "send" 8 1000 1000 1000 tcp "forced_cache")  # 8GB data, 1KB messages, 1000 msgs/batch with forced cache
LARGE_BATCH_FORCED_CACHE_POLL=$(construct_bench_command "$IGGY_BENCH_CMD" "poll" 8 1000 1000 1000 tcp "forced_cache")  # 8GB data, 1KB messages, 1000 msgs/batch with forced cache
...
SMALL_BATCH_FORCED_CACHE_SEND=$(construct_bench_command "$IGGY_BENCH_CMD" "send" 8 1000 100 10000 tcp "forced_cache")    # 8GB data, 1KB messages, 100 msgs/batch with forced cache
SMALL_BATCH_FORCED_CACHE_POLL=$(construct_bench_command "$IGGY_BENCH_CMD" "poll" 8 1000 100 10000 tcp "forced_cache")     # 8GB data, 1KB messages, 100 msgs/batch with forced cache
...
SUITES=(
    "${LARGE_BATCH_FORCED_CACHE_SEND}"
    "${LARGE_BATCH_FORCED_CACHE_POLL}"
    "${LARGE_BATCH_NO_CACHE_SEND}"
    "${LARGE_BATCH_NO_CACHE_POLL}"
    "${LARGE_BATCH_NO_WAIT_SEND}"
    "${LARGE_BATCH_NO_WAIT_POLL}"
    "${SMALL_BATCH_FORCED_CACHE_SEND}"
    "${SMALL_BATCH_FORCED_CACHE_POLL}"
...
for (( i=0; i<${#SUITES[@]} ; i+=2 )) ; do
    ...
    if [[ "$SEND_BENCH" == *"_forced_cache_"* ]] || [[ "$POLL_BENCH" == *"_forced_cache_"* ]]; then
    ...
    elif [[ "$SEND_BENCH" == *"_no_cache_"* ]] || [[ "$POLL_BENCH" == *"_no_cache_"* ]]; then
    ...
    elif [[ "$SEND_BENCH" == *"_no_wait_"* ]] || [[ "$POLL_BENCH" == *"_no_wait_"* ]]; then
