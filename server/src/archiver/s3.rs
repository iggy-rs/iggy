...
impl S3Archiver {
    ...
    async fn copy_file_to_tmp(&self, path: &str) -> Result<String, ServerArchiverError> {
        ...
        fs::create_dir_all(destination.parent().unwrap())
            ...
            .with_error(|_| {
                format!(
                    "Failed to create temporary S3 upload directory for path: {destination_path}"
                )
            })?;
        debug!("Copying file: {path} to temporary S3 upload path: {destination_path}");
        fs::copy(source, &destination).await.with_error(|_| {
            format!("Failed to copy file: {path} to temporary S3 upload path: {destination_path}")
        })?;
    ...
    async fn archive(
        ...
        for path in files {
            ...
            let mut file = file::open(&source)
                .await
                .with_error(|_| format!("Failed to open source file: {source} for archiving"))?;
            let base_directory = base_directory.as_deref().unwrap_or_default();
            ...
            if let Err(error) = response {
                ...
                fs::remove_file(&source).await.with_error(|_| {
                    format!("Failed to remove temporary file: {source} after S3 failure")
                })?;
            ...
            if status == 200 {
                ...
                fs::remove_file(&source).await.with_error(|_| {
                    format!("Failed to remove temporary file: {source} after successful archive")
                })?;
                continue;
            }
            ...
            fs::remove_file(&source).await.with_error(|_| {
                format!("Failed to remove temporary file: {source} after invalid status code")
            })?;
            return Err(ServerArchiverError::CannotArchiveFile {
                file_path: path.to_string(),
            });
