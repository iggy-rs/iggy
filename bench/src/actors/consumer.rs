...
impl Consumer {
    ...
    pub async fn run(&self) -> Result<BenchmarkIndividualMetrics, IggyError> {
        ...
        while received_messages < total_messages {
            ...
            let (_, _) = match self.polling_kind {
            ...
            if polled_messages.len() != messages_per_batch as usize {
                warn!(
            ...
            received_messages += polled_messages.len() as u64;
            ...
            if batch_user_size_bytes == 0 || batch_size_total_bytes == 0 {
                batch_user_size_bytes =
                    polled_messages[0].payload.len() as u64 * messages_per_batch as u64;
                batch_size_total_bytes =
                    polled_messages[0].get_size_bytes().as_bytes_u64() * messages_per_batch as u64;
            }
