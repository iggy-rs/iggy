...
pub async fn handle(socket: &UdpSocket, buffer: &mut [u8; 1024]) -> io::Result<()> {
    ...
    while position < length {
        ...
        let partitions = u32::from_le_bytes(payload[position + 4..position + 8].try_into().unwrap());
        let name_length = u32::from_le_bytes(payload[position + 8..position + 12].try_into().unwrap()) as usize;
        let name =  from_utf8(&payload[position + 12..position + 12 + name_length]);
        topics.push(Topic { id, partitions, name: name.unwrap().to_string() });
