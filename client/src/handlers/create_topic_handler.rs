...
pub async fn handle(input: &[&str], socket: &UdpSocket, buffer: &mut [u8; 1024]) -> io::Result<()> {
    if input.len() != PARTS {
        return Err(io::Error::new(io::ErrorKind::Other, format!("Invalid create topic command, expected {} parts.", PARTS)));
    }
    ...
    if !(1..=100).contains(&partitions_count) {
        return Err(io::Error::new(io::ErrorKind::Other, format!("Invalid number of partitions: {}, expected between 1 and 100.", partitions_count)));
    }

    let name = from_utf8(input[2].as_bytes()).unwrap();
    if name.len() > 100 {
        return Err(io::Error::new(io::ErrorKind::Other, format!("Invalid topic name: {}, expected between 1 and 100 characters.", name)));
    }
    ...
    socket.send([COMMAND, id, partitions_count, name].concat().as_slice()).await?;
